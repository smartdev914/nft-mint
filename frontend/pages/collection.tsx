...
import { getCollectionsByOwner } from "@/libs/shyft"
      ...
      <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
        {collections.map((collection) => {
          return (
            <div
              key={collection.id}
              className="relative justify-between flex w-full max-w-[20rem] flex-col rounded-xl bg-gradient-to-tr from-gray-900 to-gray-800 bg-clip-border p-8 text-white shadow-md shadow-gray-900/20"
            >
              <div>
                  ...
                  <ul className="flex flex-col gap-4">
                    <li className="flex items-center gap-4">
                      ...
                      <p className="block font-sans text-base font-normal leading-relaxed text-inherit antialiased">
                        Address:{" "}
                        {collection.collectionMint.substring(0, 5) +
                          "..." +
                          collection.collectionMint.substr(collection.collectionMint.length - 5)}
                      </p>
                    </li>
                    <li className="flex items-center gap-4">
                      <span className="rounded-full border border-white/20 bg-white/20 p-1">
                    ...
                    <li className="flex items-center gap-4">
                      ...
                      <p className="block font-sans text-base font-normal leading-relaxed text-inherit antialiased">
                        Seller Fee Basis Points: {collection.sellerFeeBasisPoints}
                      </p>
                    </li>
                  </ul>
                </div>
              ...
              <div className="z-30 mt-6 p-0">
                <a
                  ...
                  href={`https://translator.shyft.to/address/${collection.collectionMint}?cluster=devnet`}
                >
                  View detail
                </a>
              </div>
